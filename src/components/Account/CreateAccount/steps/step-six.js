...
export class StepSix extends React.Component {
    ...
    selectedCompany = id => {
        ...
        if (selectedCompanies.includes(id)) {
            let index = selectedCompanies.indexOf(id)
        ...
        return(
            <Fragment>
                <Container>
                    <FollowHeader>
                        <LeftHeader>
                            <HeaderTxt>Recommended Companies To Follow</HeaderTxt>
                        </LeftHeader>
                        <RightHeader>
                            <SearchBox>
                    ...
                    <CollegesContainer>
                        {colleges.map((college, index) => (
                            <EachColleges key={index} index={index+1} onClick={() => this.selectedCompany(college.id)}>
                                <CollegeWrap>
                                    <CollegeLogo image={college.logo}>
                                        <Marked active={selectedCompanies.includes(college.id)}>
